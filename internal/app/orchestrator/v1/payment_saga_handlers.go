...
type PaymentSagaHandler struct {
	holdFundsCmd    HoldFundsCommand
	releaseFundsCmd ReleaseFundsCommand
	debitFundsCmd   DebitFundsCommand
	authorizeCmd    AuthorizeGatewayCommand
	updateStatusCmd UpdatePaymentStatusCommand
	notifyUserCmd   NotifyUserCommand
}
...
func (h *PaymentSagaHandler) HandlePaymentCreated(ctx context.Context, event domain.PaymentCreatedEvent) error {
	log.Printf("Handling payment.created for PaymentID: %s. Attempting to hold funds.", event.ID)
	// Here you would add logic to handle errors and trigger compensation (though this is the happy path).
	return h.holdFundsCmd.Hold(ctx, event.ID, event.WalletID, event.Amount, event.Currency)
}
...
// HandleFundsDebited is triggered by a `wallet.debit` event from the Wallet Service.
...
func (h *PaymentSagaHandler) HandleFundsDebited(ctx context.Context, event domain.WalletCommandEvent) error {
	log.Printf("Handling wallet.debit for PaymentID: %s. Finalizing payment.", event.PaymentID)
